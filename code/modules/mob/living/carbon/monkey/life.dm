...
/mob/living/carbon/monkey/handle_mutations_and_radiation()
	if(radloss)
		if(radloss > RAD_MONKEY_GORILLIZE)
			if(prob((((radloss - RAD_MONKEY_GORILLIZE + RAD_MOB_GORILLIZE_FACTOR)/RAD_MOB_GORILLIZE_FACTOR)^RAD_MONKEY_GORILLIZE_EXPONENT) - 1))
				gorillize()
				return
		if(radloss > RAD_MOB_KNOCKDOWN && prob(RAD_MOB_KNOCKDOWN_PROB))
			if(!(combat_flags & COMBAT_FLAG_HARD_STAMCRIT))
				emote("collapse")
			DefaultCombatKnockdown(RAD_MOB_KNOCKDOWN_AMOUNT)
			to_chat(src, "<span class='danger'>You feel weak.</span>")
		if(radloss > RAD_MOB_MUTATE)
			if(prob(1))
				to_chat(src, "<span class='danger'>You mutate!</span>")
				easy_randmut(NEGATIVE+MINOR_NEGATIVE)
				emote("gasp")
				domutcheck()
		if(radloss > RAD_MOB_VOMIT && prob(RAD_MOB_VOMIT_PROB))
			vomit(10, TRUE)
	return ..()
