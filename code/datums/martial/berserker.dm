...
/datum/martial_art/berserker/on_remove(mob/living/carbon/human/H)
	. = ..()
	REMOVE_TRAIT(H, TRAIT_POOR_AIM, BERSERKER_TRAIT)
...
/datum/martial_art/oldserker
	name = "Teachings of Canis"
	id = MARTIALART_OLDSERKER
	allow_temp_override = FALSE
	help_verb = /mob/living/carbon/human/proc/oldserker_blows_help
	pugilist = TRUE

#define OLD_HARD_PUNCH_COMBO "HH"
#define OLD_SHOULDERCHECK_COMBO "HD"
#define OLD_CHOKE_SLAM_COMBO "HG"
...
/datum/martial_art/oldserker/check_streak(mob/living/carbon/human/A, mob/living/carbon/human/D)
	if(findtext(streak,OLD_HARD_PUNCH_COMBO))
		...
		hardPunch(A,D)
		return TRUE
	if(findtext(streak,OLD_SHOULDERCHECK_COMBO))
		streak = ""
		shoulderCheck(A,D)
		return TRUE
	if(findtext(streak,OLD_CHOKE_SLAM_COMBO))
		streak = ""
		chokeSlam(A,D)
		return TRUE
	return FALSE
...
///Gutpunch: Harm Harm, high force punch on every second harm punch, has a chance to crit for near triple damage
/datum/martial_art/oldserker/proc/hardPunch(mob/living/carbon/human/A, mob/living/carbon/human/D)
	///this var is so that the strong punch is always aiming for the body part the user is targeting and not trying to apply to the chest before deviating
	var/obj/item/bodypart/affecting = D.get_bodypart(ran_zone(A.zone_selected))
	var/armor_block = D.run_armor_check(affecting, "melee")
	A.do_attack_animation(D, ATTACK_EFFECT_PUNCH)
	...
	if(prob(10))
		...
		playsound(get_turf(D), 'sound/weapons/bite.ogg', 50, TRUE, -1)
		D.visible_message("<span class='warning'>[D] staggers as they're slammed in the stomach</span>", "<span class='userdanger'>You are struck with incredible precision by [A]!</span>")
		log_combat(A, D, "critcal hard punched (Oldserker)")//log it here because a critical can swing for 40 force and it's important for the sake of how hard they hit
	else
		playsound(get_turf(D), 'sound/weapons/punch1.ogg', 25, TRUE, -1)
		log_combat(A, D, "hard punched punched (Oldserker)")//so as to not double up on logging
	D.apply_damage(damage + 2.5 + crit_damage, BRUTE, affecting, armor_block, wound_bonus = CANT_WOUND)
	return TRUE

///Shouldercheck: Harm Harm Harm combo, throws people seven tiles backwards
/datum/martial_art/oldserker/proc/shoulderCheck(mob/living/carbon/human/A, mob/living/carbon/human/D)
	var/damage = (damage_roll(A,D) + 3)
	var/obj/item/bodypart/affecting = D.get_bodypart(BODY_ZONE_CHEST)
	var/armor_block = D.run_armor_check(affecting, "melee")
	A.do_attack_animation(D, ATTACK_EFFECT_KICK)
	...
	D.apply_damage(damage, BRUTE, BODY_ZONE_CHEST, armor_block, wound_bonus = CANT_WOUND, wound_bonus = CANT_WOUND)
	to_chat(A, "<span class='danger'>You shouldercheck [D]!</span>")
	log_combat(A, D, "shoulderchecked (Oldserker)")
	return TRUE

///chokeslam: Harm Grab combo, knocks people down, deals stamina damage while they're on the floor
/datum/martial_art/oldserker/proc/chokeSlam(mob/living/carbon/human/A, mob/living/carbon/human/D)
	var/damage = damage_roll(A,D)
	A.do_attack_animation(D, ATTACK_EFFECT_KICK)
	var/obj/item/bodypart/affecting = D.get_bodypart(BODY_ZONE_HEAD)
	var/armor_block = D.run_armor_check(affecting, "melee")
	...
	if((D.mobility_flags & MOBILITY_STAND))
		...
		D.visible_message("<span class='warning'>[A] pummels [D]!</span>", \
					"<span class='userdanger'>You are kicked in the head by [A]!</span>", "<span class='hear'>You hear a sickening sound of flesh hitting flesh!</span>", COMBAT_MESSAGE_RANGE, A)
		to_chat(A, "<span class='danger'>You pummel [D]!</span>")
	log_combat(A, D, "chokeslammed (Oldserker")
	return TRUE

/datum/martial_art/oldserker/grab_act(mob/living/carbon/human/A, mob/living/carbon/human/D)
	add_to_streak("G",D)
	if(check_streak(A,D))
		return TRUE
	log_combat(A, D, "grabbed (Oldserker)")
	return ..()

/datum/martial_art/oldserker/harm_act(mob/living/carbon/human/A, mob/living/carbon/human/D)
	add_to_streak("H",D)
	var/damage = (damage_roll(A,D) + 2)
	if(check_streak(A,D))
		return TRUE
	...
	D.apply_damage(damage, BRUTE, affecting, armor_block, wound_bonus = CANT_WOUND)
	playsound(get_turf(D), 'sound/weapons/punch1.ogg', 25, TRUE, -1)
	log_combat(A, D, "punched (Oldserker)")
	return NO_AUTO_CLICKDELAY_HANDLING | ATTACK_IGNORE_ACTION


/datum/martial_art/oldserker/disarm_act(mob/living/carbon/human/A, mob/living/carbon/human/D)
	add_to_streak("D",D)
	if(check_streak(A,D))
		return TRUE
	log_combat(A, D, "disarmed (Oldserker)")
	return ..()

/datum/martial_art/oldserker/teach(mob/living/carbon/human/H, make_temporary = FALSE)
	. = ..()
	if(!.)
		return
	...
	H.physiology.burn_mod *= 0.9 //10% better burn resist.


/datum/martial_art/oldserker/on_remove(mob/living/carbon/human/H)
	. = ..()
	REMOVE_TRAIT(H, TRAIT_NOGUNS, OLDSERKER_TRAIT)
	REMOVE_TRAIT(H, TRAIT_PIERCEIMMUNE, OLDSERKER_TRAIT)
	REMOVE_TRAIT(H, TRAIT_NODISMEMBER, OLDSERKER_TRAIT)
	REMOVE_TRAIT(H, TRAIT_BERSERKER, OLDSERKER_TRAIT)
	H.physiology.stamina_mod = initial(H.physiology.stamina_mod)
	H.physiology.stun_mod = initial(H.physiology.stun_mod)
	H.physiology.brute_mod = initial(H.physiology.brute_mod)
	H.physiology.burn_mod = initial(H.physiology.burn_mod)

/mob/living/carbon/human/proc/oldserker_blows_help()
	set name = "Recall Teachings"
	set desc = "Remember the sacred rites."
	set category = "Rites"

	to_chat(usr, "<b><i>You retreat inward and recall the rites.</i></b>")

	to_chat(usr, "<span class='notice'>Gutpunch</span>: Harm Harm. Deal additional damage every second punch, with a chance for even more damage!")
	to_chat(usr, "<span class='notice'>Shoulder Check</span>: Harm Disarm. Launch people brutally across rooms, and away from you.")
	to_chat(usr, "<span class='notice'>Chokeslam</span>: Harm  Grab. Chokeslam to the floor. Against prone targets, deal additional stamina damage and disarm them.")
	to_chat(usr, "<span class='notice'>In addition, your body is better conditioned, giving you further stamina, increased stun resistance and better overall durability.</span>")
