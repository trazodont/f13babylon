...
/mob/living/carbon/human/verb/SetProfilePic()
	...
	var/input = stripped_input(usr,"Right click an image from discord (do not expand the image by clicking it) and click 'Copy Link' and paste it here. Must be a png. Preferred image size: 500x500 or smaller.")
	if(length(input))
